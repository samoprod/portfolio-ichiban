...
if($('.mainslider').length>0){
	$('.mainslider').slick({
		...
		nextArrow:'<button type="button" class="slick-next"><svg class="icon" viewBox="0 0 30 50"><use xlink:href="#icon-r-arrow"></use></svg></button>',
		prevArrow:'<button type="button" class="slick-prev"><svg class="icon" viewBox="0 0 30 50"><use xlink:href="#icon-l-arrow"></use></svg></button>',
		responsive: [{
			breakpoint: 768,
			settings: {
				arrows: false
...
function adaptive_header(w,h) {
		var headerCatalog=$('.header-catalog__wrap');
		var headerPhone=$('.header-line__phone');
	if(w<992.98){
		if(!headerPhone.hasClass('done')){
			headerPhone.addClass('done').appendTo(headerCatalog);
		}
		if(!$('.header-top__list').hasClass('done')){
			$('.header-top__list').addClass('done').insertAfter('.mobile-menu__menutitle');
		}
		if(!$('.header-catalog__list').hasClass('done')){
			$('.header-catalog__list').addClass('done').insertAfter('.mobile-menu__catalogtitle');
		}
	}else{
		if(headerPhone.hasClass('done')){
			headerPhone.removeClass('done').insertAfter('.header-line__logo');
		}
		if($('.header-top__list').hasClass('done')){
			$('.header-top__list').removeClass('done').prependTo('.header-top__wrap');
		}
		if($('.header-catalog__list').hasClass('done')){
			$('.header-catalog__list').removeClass('done').insertBefore('.header-catalog__wrap');
		}
	}

	/*
		<h2 class="mobile-menu__menutitle">Меню</h2>
		<h2 class="mobile-menu__catalogtitle">Каталог</h2>
	*/
}
